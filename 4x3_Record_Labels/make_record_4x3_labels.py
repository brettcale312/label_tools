...
def draw_label(c, row):
    title, artist, label, year, genre, price, inv_id, barcode_num = [normalize_text(col) for col in row]
    ...
    # Artist
    if artist.strip():
        y = wrap_text(c, f"\u2022 Artist: {artist.strip()}", x, y, MAX_TEXT_W, leading=11, font_name="Helvetica", font_size=11)
        y -= 3

    # Label
    if label.strip():
        y = wrap_text(c, f"\u2022 Label: {label.strip()}", x, y, MAX_TEXT_W, leading=11, font_name="Helvetica", font_size=11)
        y -= 3

    # Year
    if year.strip():
        y = wrap_text(c, f"\u2022 Year: {year.strip()}", x, y, MAX_TEXT_W, leading=11, font_name="Helvetica", font_size=11)
        y -= 3

    # Genre
    if genre.strip():
        y = wrap_text(c, f"\u2022 Genre: {genre.strip()}", x, y, MAX_TEXT_W, leading=11, font_name="Helvetica", font_size=11)
        y -= 3
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        inp = here / "record_label_input.txt"
        outp = here / "record_4x3_label.pdf"
